...
main;
tic;
dag_gbn=learn_struct_mcmc(data_trainl',node_sizes);%MCMC（Markov Chain Monte Carlo）
MCMC_t2=toc;
fprintf('MCMC（Markov Chain Monte Carlo）算法训练%d个样本%d个结点所用的时间为%d秒\n',M,N,MCMC_t2);
dag_gbn=cell2mat(dag_gbn(N:N));%将学习后的转换成double
figure;
draw_graph(dag_gbn);
title('结构学习—MCMC 算法')
...
%% %%%推断%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%使用最大似然法学习的网络bnet3
engine=gibbs_sampling_inf_engine(bnet3);
fs3=zeros(1,N);
for Node3=1:N
...
engine=gibbs_sampling_inf_engine(bnet4);
fs4=zeros(1,N);
for Node4=1:N
    fprintf('目前计算的结点为结点%d\n',Node4);
       tic;
